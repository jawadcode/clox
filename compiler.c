...
/* A Parser Rule
         - "prefix" is a pointer to a parser function for the token if it were a
   prefix
         - "info" is a pointer to a parser function for the token if it were
   infix
         - "precedence" is the parsing precedence of the token relative to other
   tokens
 */
typedef struct {
  ParseFn prefix;
...
/* References a stack value in an outer scope
         - "index" is the location of the value on the stack
 */
typedef struct {
  uint8_t index;
  bool isLocal;
...
/* The two different types of function:
         ...
         - "TYPE_SCRIPT" is the top level of a script but is still treated as a
   function
 */
typedef enum {
  TYPE_FUNCTION,
...
/* Compiler state
         ...
         - "upvalues" is a list of upvalues for the current function
...
static void emitLoop(int loopStart) {
  emitByte(OP_LOOP);
  // Size of loop: end minus start plus the size of the "OP_LOOP" instruction
  // itself
...
// Takes in token "name" and adds its lexeme to the chunk's constants table as a
// string its index in the table is then returned
...
// Walk list of locals backwards to find most recently declared variable with
// the identifier
...
static void declareVariable() {
  ...
  // Walk current scope's list of locals backwards to check the variable has
...
// Requires next token to be identifier and then returns index of constant in
// table from "identifierConstant()"
...
static void and_(bool canAssign) {
  // "and" operator short circuits (if the left operand is false) so there needs
  // to be a jump if false
...
static void forStatement() {
  // Want to wrap entire loop in scope to prevent counter being accessed outside
  // the loop
  beginScope();
  ...
  if (!match(TOKEN_RIGHT_PAREN)) {
    // Emit jump to the end of the loop (because increment should run after the
    // body)
...
static void whileStatement() {
  // Position of the start of the loop (in terms of bytecode) so we can actually
  // loop
  int loopStart = currentChunk()->count;
